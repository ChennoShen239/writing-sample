...
p1 = plot(x, q_low, label = "Low")
plot!(p1, x, q_high, label = "High")
plot!(
    p1,
...
savefig(p1, "Result/Figs/full_info/bond_price_schedule.pdf")


# Value functions plot
p2 = plot(ae.Bgrid, ae.vf[:, iy_low], label = "Low")
plot!(p2, ae.Bgrid, ae.vf[:, iy_high], label = "High")
plot!(p2, xlabel = L"B", ylabel = L"V(y,B)", title = "Value functions", legend = :topleft)
savefig(p2, "Result/Figs/full_info/value_functions.pdf")
...
# simulate
T = 50000
y_vec, B_vec, q_vec, default_vec = AEsimulate(ae, T)
...
for i = 1:3
    ...
    for j = 1:length(def_start)
        plot!(
            plots[i],
            [def_start[j], def_end[j]],
            fill(maximum(y_vals[i]), 2),
            fillrange = [extrema(y_vals[i])...],
            fcolor = :grey,
            falpha = 0.3,
            label = "",
        )
...
savefig(plots, "Result/Figs/full_info/simulation_results.pdf")

p4 = heatmap(
    ae.Bgrid[1:(end-1)],
    ae.ygrid[2:end],
    reshape(clamp.(vec(ae.defprob[1:(end-1), 1:(end-1)]), 0, 1), ae.nB - 1, ae.ny - 1)',
)
plot!(
    xlabel = L"B^\prime",
    ylabel = L"y",
...
savefig(p4, "Result/Figs/full_info/Probability_Default.pdf")
...
rates_low_income = zeros(ae.nB)
for ib = 1:ae.nB
    # Get the optimal policy for B_prime (index)
    b_prime_idx = Int(ae.policy[ib, iy_low])
    # Get the corresponding bond price q(B_prime, y_low)
    q_val = ae.q[b_prime_idx, iy_low]
    # Calculate interest rate, handling potential q_val = 0
    if q_val > 1e-12 # Avoid division by zero or extremely small q
        rates_low_income[ib] = (1 / q_val) - 1
    else
        rates_low_income[ib] = NaN # Or some large number if preferred
    end
end


# Calculate interest rates for high income
rates_high_income = zeros(ae.nB)
for ib = 1:ae.nB
    # Get the optimal policy for B_prime (index)
    b_prime_idx = Int(ae.policy[ib, iy_high])
    # Get the corresponding bond price q(B_prime, y_high)
    q_val = ae.q[b_prime_idx, iy_high]
    # Calculate interest rate
    if q_val > 1e-12
        rates_high_income[ib] = (1 / q_val) - 1
    else
        rates_high_income[ib] = NaN
    end
end


# Generate plot
p_interest_rate = plot(ae.Bgrid, rates_low_income, label = "Low Income", linewidth = 2)
plot!(p_interest_rate, ae.Bgrid, rates_high_income, label = "High Income", linewidth = 2)
plot!(
    p_interest_rate,
    ...
    ylims = (0, 0.6), # Set y-axis limits
)

# Save the plot
savefig(p_interest_rate, "Result/Figs/full_info/equilibrium_interest_rate.pdf")
...
savings_low_income = zeros(ae.nB)
for ib = 1:ae.nB
    # Get the optimal policy for B_prime (index)
    b_prime_idx = Int(ae.policy[ib, iy_low])
    # Convert index to B_prime value
    savings_low_income[ib] = ae.Bgrid[b_prime_idx]
end

# Get savings (B') for high income
savings_high_income = zeros(ae.nB)
for ib = 1:ae.nB
    # Get the optimal policy for B_prime (index)
    b_prime_idx = Int(ae.policy[ib, iy_high])
    # Convert index to B_prime value
    savings_high_income[ib] = ae.Bgrid[b_prime_idx]
end

# Generate plot
p_savings = plot(ae.Bgrid, savings_low_income, label = "Low Income", linewidth = 2)
plot!(p_savings, ae.Bgrid, savings_high_income, label = "High Income", linewidth = 2)
...
plot!(
    ...
    xlabel = L"Current Assets ($B$)",
    ylabel = L"Next Period's Assets ($B'$)",
...
# Save the plot
savefig(p_savings, "Result/Figs/full_info/savings_function.pdf")
