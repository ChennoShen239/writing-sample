...
legend({'Spread', 'Mean'}, 'Location', 'best');
...
grid on;
formatFigure;
legend({'Debt', 'Mean'}, 'Location', 'best');
...
xlim([0 0.5]);
formatFigure;
legendLabels = arrayfun(@(y) sprintf('y = %.2f', y), yGrid(fixYs), 'UniformOutput', false);
...
grid on;
formatFigure;
legend([legendLabels(:); {'45-degree'}], 'Location', 'best');