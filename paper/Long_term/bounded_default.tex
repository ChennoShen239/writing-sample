...
\paragraph{Posterior and effective dispersion}
With Gaussian signals, the posterior variance of $\sigma$ is $(\bar
	v_\sigma^{-1}+a_\sigma)^{-1}$ for prior variance $\bar v_\sigma$. I map this
posterior dispersion to the pricing operator via a monotone function $g$:
\begin{equation}
	\theta_{\mathrm{RI}}(y,B') = 1 + \varphi\, g\!\Big( (\bar v_\sigma^{-1}+a_\sigma(y,B'))^{-1} \Big),\qquad \varphi>0,\ g'>0.
	\label{eq:theta_RI_micro}
\end{equation}
Prices remain fixed points of the \emph{same} operator evaluated at
$\theta_{\mathrm{RI}}$:
\begin{equation}
...
\end{equation}
Denote by $\mathcal S(y,B')\ge0$ the default-sensitivity of payouts to
dispersion beliefs in state $(y,B')$. The first-order condition implies
\begin{equation}
	a_\sigma(y,B') = \kappa_\sigma^{-1}\, \mathcal S(y,B'),\qquad
	\frac{\partial a_\sigma}{\partial \text{stress}} > 0 \ \Rightarrow\ \frac{\partial \theta_{\mathrm{RI}}}{\partial \text{stress}} > 0.
	\label{eq:foc_sigma_micro}
\end{equation}
Hence attention to dispersion—and thus the PRO intensity—rises in high-debt/low-$y$
states where price schedules are steeper in default probability.

\begin{proposition}[Operator properties and inherited comparative statics]
	\label{prop:RI_operator_micro}
	Suppose $\Phi$ is strictly convex and $g$ in~\eqref{eq:theta_RI_micro} is
...
\begin{proof}
	...
	$\mathcal T_{\theta_{\mathrm{RI}}(\cdot)}$ by monotonicity of $g$ and continuity of
	$a_\sigma(y,B')$ from~\eqref{eq:foc_sigma_micro}. Fixed-point arguments on
...
credibility and precision of official macro signals. In
\eqref{eq:att_opt_micro}–\eqref{eq:foc_sigma_micro}, a fall in the productivity
of mean information (effective $a_\mu$) raises the marginal value of learning
about dispersion, pushing up $a_\sigma$ and therefore $\theta_{\mathrm{RI}}$
via \eqref{eq:theta_RI_micro}. The model predicts: (i) higher average spreads;
(ii) a steeper price pivot; (iii) cross-peer “decoupling” when others'
dispersion signals did not deteriorate; and (iv) a higher default threshold
with concurrent deleveraging—matching the event-study patterns in the Argentina
episode.
