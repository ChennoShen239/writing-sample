...
stress.
Intuitively, PRO tilts pricing so that borrowing becomes costlier in the range
where governments normally issue and slightly cheaper only very close to
default. In the usual borrowing region (below the pivot threshold) the
...
\citep{KlibanoffMarinacciMukerji2005} and \textit{variational preferences}
\citep{MaccheroniMarinacciRustichini2006}---likewise tilt decision weights
...
aligned comparative statics.\footnote{This pivot effect differs from both
	...
	\citep{AmadorPhelan2021} often generate monotonic price effects through type
	revelation under standard implementations, and \citep{Fourakis2021} shows
	...
	models like \citep{RochRoldan2023} explain high spreads through a uniform
	distortion of fundamental shocks, which also differs from the pivot mechanism.}
And unlike \textit{diagnostic expectations} \citep{GennaioliShleifer2018,
...
standard formulations may find challenging to replicate.\footnote{The empirical
	...
	\citep{MorelliMoretti2023} find that misreporting episodes increase spreads on
	all debt instruments, the mechanism here implies divergent effects across the
	debt distribution. Similarly, while \citep{AmadorPhelan2023} shows reputation
...
\paragraph{Roadmap} Section~\ref{sec:model} presents the environment and recursive equilibrium;
...
formulation yields identical monotone comparative statics in precisions; see \citealp{Sims2003, MackowiakWiederholt2009, MatejkaMcKay2015, Veldkamp2011}.)
...
\begin{remark}[Relation to ambiguity/robust preferences]
	Preference-based ambiguity models---smooth ambiguity \citep{KlibanoffMarinacciMukerji2005}, variational preferences \citep{MaccheroniMarinacciRustichini2006}, and robust control \citep{HansenSargent2008}---can be represented as likelihood twists or penalty-adjusted evaluations that effectively tilt tails in the same pricing operator. Under the operator assumptions in Appendix~\ref{app:operator_analysis}, the pivot and policy comparative statics follow analogously. The RI formulation here endogenizes the tilt through attention choice, yielding the state-dependent $\theta_{\mathrm{RI}}(\cdot)$.
\end{remark}
