...
\begin{proposition}[Existence and Uniqueness of the Recursive MPE]
...
\begin{proposition}[Welfare Loss under Lender Pessimism]
	\label{prop:welfare}
	Let $V_i(y,B)$ denote the sovereign's ex-ante equilibrium value function in economy $i\in\{1,\theta\}$, where $\theta>1$ indexes pessimistic lenders and $\theta=1$ rational lenders.
	...
	Then equilibrium welfare is strictly lower under pessimistic lenders:
	\[
		V_\theta(y,B) \;<\; V_1(y,B).
	\]
...
Proposition~\ref{prop:welfare} shows that lender pessimism reduces equilibrium
welfare. A natural question is whether an optimal fiscal authority can undo
this loss. We study a Ramsey planner who can choose lump-sum taxes/transfers
but takes the bond pricing mechanism as given.

Lender pessimism acts like a persistent wedge in the intertemporal price of
borrowing. Transfers can reallocate resources within and across periods subject
to a zero present-value constraint, but they cannot change the shadow price at
which the government trades across dates. Hence the pessimistic price schedule
distorts debt choices even under optimal fiscal policy.
...
\begin{proposition}[Ramsey welfare loss under pessimistic prices]
...
defaults relative to unexpected repayments---can make pessimism persistent and
self-reinforcing. Rare but salient defaults move beliefs more than frequent,
modestly good realizations, creating a drift toward pessimism.
...
\begin{proposition}\label{prop:endogenous_beliefs}
	...
	\begin{enumerate}
		\item[\textbf{(i)}] $\E[\Theta^*]>1$ (persistent pessimism);
...
\begin{proposition}\label{prop:optimal_communication}
	...
	\begin{enumerate}
		\item[\textbf{(i)}] There exists a unique $\alpha^*\in[0,1]$ satisfying the FOC
		      ...
		      \end{equation}
		\item[\textbf{(ii)}] If $\partial W/\partial \vartheta<0$ and $\partial \theta_{\mathrm{eff}}/\partial \alpha=1-\theta<0$, then $\partial \alpha^*/\partial \theta>0$: more pessimism raises optimal transparency.
...
\begin{proof}
	...
	\end{align}
	Intuitively: when repayment strictly dominates default (\(\Delta V>0\)), pessimistic lenders assign a \emph{higher} default probability; when default dominates (\(\Delta V<0\)), they assign a \emph{lower} default probability.
...
\begin{proof}
	...
	q_i/\partial y>0\). The sensitivity is dampened under pessimism because the
	mapping \(\Delta V \mapsto P_\theta=L(-\Delta V/(\theta\eta))\) is flatter when
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item[(A2)] (Continuation marginal ordering.) Increasing next-period debt lowers the continuation value and it does so \emph{more} under pessimistic lenders:
