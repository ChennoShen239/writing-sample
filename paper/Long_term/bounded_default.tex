...
% (Moved: see microfoundations section after Eq.~\eqref{eq:foc_sigma_micro})
...
This section microfounds PRO via rational inattention (RI). Competitive lenders
choose how to allocate limited attention across mean and dispersion signals, at
convex information-processing costs. Their choice endogenizes a state-dependent
tail-weight parameter $\theta_{\mathrm{RI}}(s)$ that we evaluate inside the
\emph{same} pricing operator as in the baseline, so all results are genuine
comparative statics within a single, unified environment.

\paragraph{Motivation and linkage}
The goal is to show that the baseline PRO parameter $\theta$ is not an
exogenous belief shifter but the endogenous outcome of optimal attention under
costs. The key discipline is the \emph{same-operator} principle: the
state-dependent tail weight $\theta_{\mathrm{RI}}(s)$ enters directly into the
baseline pricing operator $\mathcal T_\theta$
(Definition~\ref{def:pricing_operator}). Thus, the microfoundation preserves
primitives, debt structure, and the sovereign’s aggregator $J_\rho$; only the
evaluation point of the \emph{same} operator changes through information
choice. Let $s=(y,B')$ denote the state. For the operator properties used here
and in Appendix~\ref{app:operator_analysis}, restrict $\theta\in[1,\bar\theta]$
with finite $\bar\theta>1$.

\paragraph{Economic environment and information structure}
...
Signals do not augment the state or alter primitives. Mean attention $a_\mu$
refines the posterior over the \emph{same} fundamentals that shift $\Delta V$;
dispersion attention $a_\sigma$ only moves the tail-weight parameter at which
the \emph{same} pricing operator is evaluated, $\theta_{\mathrm{RI}}(y,B')$.
Setting $a_\sigma\equiv0$ (hence $\theta_{\mathrm{RI}}\equiv1$) recovers the
baseline with rational lenders.

This information environment will feed into pricing through the attention–tilt
mapping below: attention determines effective information, which determines the
tail-weight in default beliefs that the same pricing operator uses.

\paragraph{Core mechanism: from attention to tail weight and pricing}
...
\end{equation}
\begin{remark}[Why $a_\sigma\mapsto \theta_{\mathrm{RI}}$ is increasing]
	In the logistic default mapping of~\eqref{eq:net_value_definition}, $\theta$ scales the within‑period shock in the denominator: \(P_\theta=L(-\Delta V/(\theta\eta))\). Raising $\theta$ flattens the logistic around the decision margin $\Delta V\approx 0$, pushing probabilities toward $1/2$: safer states look a bit riskier (default probability up when $\Delta V>0$) and doomed states look a bit less certain (default probability down when $\Delta V<0$). This is exactly what it means to put \emph{more weight on randomness near the threshold}.

	The dispersion attention $a_\sigma$ is the resource devoted to processing
	instability/second‑moment information. Allocating more $a_\sigma$ is valuable
	only insofar as it changes prices via the "threshold‑randomness" channel. The
	reduced‑form that implements this, while keeping the \emph{same} pricing
	operator, is to evaluate it at a larger $\theta$. Two complementary points pin
	down the monotonicity: - By construction (see the definition of $\mathcal S$
	below), the marginal value of tail‑weight in pricing is nonnegative, \(\mathcal
	S(y,B')=\E[\partial U/\partial\theta]\ge 0\). The lender’s FOC \(\kappa_\sigma
	a_\sigma=\varphi\,\mathcal S\) then says: whenever an extra bit of tail‑weight
	raises expected payouts, the optimal response is to raise $a_\sigma$.
	Consistency of signs requires that $a_\sigma\mapsto\theta$ be
	\emph{increasing}. - Geometrically, as $a_\sigma$ rises, the lender
	intentionally broadens the perceived decision noise exactly where payoffs are
	most sensitive (around $\Delta V\approx 0$). In the logit, the unique
	one‑parameter way to broaden around the center while preserving the functional
	form is to increase $\theta$.
\end{remark}
\medskip
\paragraph{A local bridge: posterior mixtures soften the logit}
Let $z\equiv -\Delta V/\eta$ and $P(z)=L(z)$. If beliefs are a mean-zero
mixture $z+\varepsilon$ with $\Var(\varepsilon)=\nu$ (reflecting how processing
and emphasizing dispersion risk affects the perceived margin), then small
dispersion softens slopes around the threshold in the same way as a higher
logit temperature.

\begin{lemma}[RI mixture $\Rightarrow$ logit temperature, local]
	\label{lem:ri_logit_bridge}
	There exists a function $\tilde\theta(\nu)=1+\tfrac{\nu}{4}+o(\nu)$ as
	$\nu\to0$ such that, uniformly on $|z|\le \bar z$,
	\[
		\E\big[L(z+\varepsilon)\big] \;=\; L\!\Big(\tfrac{z}{\tilde\theta(\nu)}\Big)\; +\; O\big(\nu^{3/2}\big).
	\]
	In particular, at the margin $z=0$, the slope satisfies $\E[L'(\varepsilon)] =
		\tfrac{1}{4}\big(1-\tfrac{\nu}{4}\big) + o(\nu)$, which matches a logit with
	temperature $\tilde\theta$ via $L'_\theta(0)=\tfrac{1}{4\theta}$. Identifying
	$\nu=\Var(\varepsilon)=\Var(\xi)/\eta^2$ for an implied uncertainty on $\Delta
		V$, small posterior dispersion is locally equivalent to evaluating the same
	operator at $\theta=\tilde\theta>1$.
\end{lemma}

\noindent\textit{Proof sketch.} Third-order Taylor around $z=0$ using
$L'(0)=1/4$, $L''(0)=0$, $L'''(0)=-1/8$ yields
$\E[L'(\varepsilon)] = L'(0) + \tfrac{\Var(\varepsilon)}{2}L'''(0)+o(\nu)$.
Slope matching with $L'(0)/\tilde\theta$ delivers the expansion for
$\tilde\theta(\nu)$. A standard remainder bound gives the uniform approximation
on compact neighborhoods. Full details are in Appendix~\ref{app:ri_logit_bridge}.
...
Thus micro decisions about attention enter prices only by changing the
parameter at which the same operator is evaluated—no new pricing friction is
introduced relative to the baseline.
...
\paragraph{Choice problem and first-order conditions}
...
\end{equation}
\noindent\textit{Lenders' objective.} For a given state $(y,B')$ and price function
$q$, define the ex-ante value of a unit face-value bond under attention
$(a_\mu,a_\sigma)$ by
\begin{equation}
	\label{eq:lender_U_def}
	U(y,B';a_\mu,a_\sigma; q)
	\;\coloneqq\; \frac{1}{1+r}\;\E_{s_\mu\mid y;\,a_\mu}\;\E_{y'\mid y, s_\mu;\,a_\mu}\!\left[\bigl(1-P_{\theta_{\mathrm{RI}}(y,B';a_\sigma)}(y',B')\bigr)\Bigl(\kappa+(1-\delta)\,\E_{B''\mid y',B'}\big[\,q(y',B'')\,\big]\Bigr)\right],
\end{equation}
where the inner expectation is taken under the posterior for $y'$ implied by the
Gaussian signal $s_\mu$ with precision $\psi_\mu a_\mu$ (for concreteness, take
$\mu\equiv y'$), and $P_{\theta}$ is the logistic default probability in
\eqref{eq:net_value_definition}. Prices in equilibrium continue to be the fixed
point of the \emph{same} operator evaluated at $\theta_{\mathrm{RI}}(\cdot)$; the
objective in~\eqref{eq:lender_U_def} formalizes the pre-signal valuation that
determines the marginal benefits of attention.
...
\paragraph{Key results: comparative statics}
The next corollary records the implied state monotonicity of attention and the
RI tail weight.
...
Under the linear mapping and the FOC for $a_\sigma$, we further obtain the
closed form
\begin{equation}
...
Beyond state monotonicity, it is crucial to establish cross-channel
substitution: when credibility of mean information falls, optimal mean
attention declines while dispersion attention rises. The next result formalizes
this effect within the same optimization problem.

\begin{proposition}[Attention substitution under credibility shifts]\label{prop:attention_substitution}
	Fix a state $s=(y,B')$ and write $a_\mu^{\mathrm{eff}}\coloneqq \psi_\mu a_\mu$. Let
	\(\mathcal M(a_\mu^{\mathrm{eff}},a_\sigma)\coloneqq \E\big[\partial U/\partial a_\mu^{\mathrm{eff}}\big]\)
	and \(\mathcal S(a_\mu^{\mathrm{eff}},a_\sigma)\coloneqq \E\big[\partial U/\partial \theta\big]\).
	Assume $\mathcal M,\mathcal S\in C^1$ and for all relevant arguments the following hold:
	\begin{enumerate}
		\item[(A1)] Diminishing returns: $\mathcal M_{\mu}\le0$, $\mathcal S_{\sigma}\le0$.
		\item[(A2')] Posterior-concentration effect (primitive): $\mathcal S_{\mu}\le0$.
		\item[(A3)] Strict concavity of the attention problem in $(a_\mu,a_\sigma)$ at the interior maximizer:
		      \[
			      \kappa_\mu-\psi_\mu^{2}\,\mathcal M_{\mu} \,>\,0,\qquad \kappa_\sigma-\varphi\,\mathcal S_{\sigma} \,>\,0,
		      \]
		      and
		      \[
			      \bigl(\kappa_\mu-\psi_\mu^{2}\,\mathcal M_{\mu}\bigr)\bigl(\kappa_\sigma-\varphi\,\mathcal S_{\sigma}\bigr)
			      \;>\; 0.
		      \]
		\item[(A4)] Productivity raises the marginal return to mean attention: $\mathcal M+\psi_\mu a_\mu\,\mathcal M_{\mu}>0$.
	\end{enumerate}
	Then the unique interior solution $(a_\mu^\ast,a_\sigma^\ast)$ to the first-order conditions
	\eqref{eq:foc_sigma_micro} and~\eqref{eq:foc_mu_micro} satisfies
	\[
		\frac{\partial a_\sigma^\ast}{\partial \psi_\mu}\;\le\;0.
	\]
	Moreover, if in addition $\mathcal M_{\sigma}\le0$, then also
	\[
		\frac{\partial a_\mu^\ast}{\partial \psi_\mu}\;\ge\;0.
	\]
	Strict inequalities obtain when the relevant weak inequalities are strict.
\end{proposition}
...
\medskip
\paragraph{Closing the loop: same operator and empirics}
These continuity and monotonicity properties feed directly into the operator
that prices bonds.
...
\paragraph{Mean-attention channel and pivot invariance}
Mean attention $a_\mu$ refines the posterior over fundamentals that already
enter $\Delta V$. To keep the same-operator discipline, I separate channels:
$a_\sigma$ moves the tail-weight $\theta$ used by the \emph{same} pricing
operator, while $a_\mu$ can (in principle) shift the location of $\Delta V$.
The core pivot results are robust to such shifts.

\begin{proposition}[Mean attention and price-pivot invariance]
	\label{prop:mean_attention_pivot}
	Suppose $\Delta V(y,B';a_\mu^{\mathrm{eff}})$ preserves the baseline
	monotonicity ($\partial_y\Delta V>0$, $\partial_{B'}\Delta V<0$) uniformly in
	$a_\mu^{\mathrm{eff}}$, and remains continuous and bounded. Then under the
	operator conditions in Appendix~\ref{app:operator_analysis}, the
	single-crossing price pivot in $B'$ at $B^*(y)$, its monotonicity in $y$, and
	the deleveraging/high-threshold comparative statics continue to hold with
	$\theta$ replaced by $\theta_{\mathrm{RI}}(y,B')$ and $\Delta V$ replaced by
	$\Delta V(y,B';a_\mu^{\mathrm{eff}})$.
\end{proposition}

\noindent\textit{Proof sketch.} Positivity and order preservation of
$\mathcal T_\theta$ extend to $\mathcal T_{\theta_{\mathrm{RI}}(\cdot)}$.
Monotone changes in $\Delta V$ retain single crossing in the logistic argument,
so the pivot characterization and its monotone comparative statics survive.
Details are in Appendix~\ref{app:mean_attention_pivot}.

\paragraph{Intuition for Assumption (A2').}
Assumption (A2') requires $\mathcal S_\mu\le0$: when mean information becomes
more precise, the \emph{marginal} value of dispersion attention falls. The
intuition is kernel-based: the price sensitivity to $\theta$ is proportional to
weights like $L'\!\left(-\Delta V/(\theta\eta)\right)$, which peak at the
threshold and decay as $|\Delta V|$ grows. Sharper mean posteriors reduce mass
near the threshold, shrinking the integral that defines $\mathcal S$. A
two‑point illustration: if $\Delta V\in\{+m,-m\}$ with posterior weight $\pi$
on $+m$, then the contribution to $\mathcal S$ is
$\pi(1-\pi)\,L'\!\left(m/(\theta\eta)\right)$, maximized at $\pi=\tfrac12$;
increasing mean precision pushes $\pi$ away from $\tfrac12$, reducing the
marginal return to extra tail‑tilt. A formal statement is in
Appendix~\ref{app:attention_substitution}.

\paragraph{Operator pipeline at a glance}
For clarity, the static pricing fixed point with endogenous attention can be
viewed as the composition
\[
	\quad q \;\xrightarrow{\,J_\rho\,}\; U \;\xrightarrow{\,(\mathcal M,\mathcal S)\,}\;
	(a_\mu,a_\sigma) \;\xrightarrow{\,a_\sigma\mapsto\theta_{\mathrm{RI}}\,}\;
	\mathcal T_{\theta_{\mathrm{RI}}(\cdot)}[q] \, ,
\]
with the continuity and boundedness conditions in
Appendices~\ref{app:operator_analysis},\ref{app:ri_general} ensuring a unique
and well-behaved equilibrium price schedule.
...
In short, credibility shocks (lower $\psi_\mu$) cascade through endogenous
attention—reducing $a_\mu^*$, raising $a_\sigma^*$, and increasing
$\theta_{\mathrm{RI}}$—and all pricing remains governed by the same operator as
in the baseline. This makes the microfoundation fully consistent with the
comparative statics developed earlier.
...
\subsection{Proof of Proposition~\ref{prop:attention_substitution}}\label{app:attention_substitution}
\begin{proof}
	Suppress the state $s$ for notational clarity. Define the two first-order conditions as
	\[
		F_1(a_\mu,a_\sigma,\psi_\mu)\;\coloneqq\; \kappa_\sigma a_\sigma-\varphi\,\mathcal S(\psi_\mu a_\mu,a_\sigma)=0,
	\]
	\[
		F_2(a_\mu,a_\sigma,\psi_\mu)\;\coloneqq\; \kappa_\mu a_\mu-\psi_\mu\,\mathcal M(\psi_\mu a_\mu,a_\sigma)=0.
	\]
	Let subscripts denote partial derivatives with respect to the indicated
	argument. The Jacobian with respect to $(a_\mu,a_\sigma)$ is
	\[
		J\;=\;\begin{bmatrix}
			\partial_{a_\mu}F_1 & \partial_{a_\sigma}F_1 \\[2pt]
			\partial_{a_\mu}F_2 & \partial_{a_\sigma}F_2
		\end{bmatrix}
		\;=\;
		\begin{bmatrix}
			-\varphi\,\psi_\mu\,\mathcal S_{\mu}      & \kappa_\sigma-\varphi\,\mathcal S_{\sigma} \\[2pt]
			\kappa_\mu-\psi_\mu^{2}\,\mathcal M_{\mu} & -\psi_\mu\,\mathcal M_{\sigma}
		\end{bmatrix}.
	\]
	By (A1)–(A3), $\kappa_\sigma-\varphi\,\mathcal S_{\sigma}>0$ and
	$\kappa_\mu-\psi_\mu^{2}\,\mathcal M_{\mu}>0$, and the negative definiteness of
	the Hessian of the attention objective is equivalent to
	\[\det J \,=\, \varphi\,\psi_\mu^{2}\,\mathcal S_{\mu}\,\mathcal M_{\sigma}-\bigl(\kappa_\sigma-\varphi\,\mathcal S_{\sigma}\bigr)\bigl(\kappa_\mu-\psi_\mu^{2}\,\mathcal M_{\mu}\bigr) \;<\;0,\]
	so $J$ is nonsingular. Differentiate $F_1=F_2=0$ with respect to $\psi_\mu$ and
	solve via the Implicit Function Theorem:
	\[
		\begin{bmatrix} \partial a_\mu^*/\partial\psi_\mu \\ \partial a_\sigma^*/\partial\psi_\mu \end{bmatrix}
		\;=\; - J^{-1}\,\begin{bmatrix} \partial_{\psi_\mu}F_1 \\ \partial_{\psi_\mu}F_2 \end{bmatrix}.
	\]
	Compute the parameter derivatives:
	\[\partial_{\psi_\mu}F_1 = -\varphi\,a_\mu\,\mathcal S_{\mu} \;\ge\;0\quad\text{by (A2')},\]
	\[\partial_{\psi_\mu}F_2 = -\bigl(\mathcal M+\psi_\mu a_\mu\,\mathcal M_{\mu}\bigr) \;<\;0\quad\text{by (A4)}.\]
	Using the explicit inverse of a $2\times2$ matrix, the $a_\sigma$ derivative is
	\[
		\frac{\partial a_\sigma^*}{\partial\psi_\mu}
		\;=\;\frac{1}{\det J}\Bigl[\bigl(\kappa_\mu-\psi_\mu^{2}\,\mathcal M_{\mu}\bigr)\,\partial_{\psi_\mu}F_1 \;+\; \varphi\,\psi_\mu\,\mathcal S_{\mu}\,\partial_{\psi_\mu}F_2\Bigr].
	\]
	Here $\det J<0$, $\kappa_\mu-\psi_\mu^{2}\,\mathcal M_{\mu}>0$,
	$\partial_{\psi_\mu}F_1\ge0$, and $\mathcal S_{\mu}\le0$ while
	$\partial_{\psi_\mu}F_2<0$, so the numerator is nonnegative and strictly
	positive if (A2') is strict. Therefore $\partial
		a_\sigma^*/\partial\psi_\mu\le0$ (strictly $<0$ under strict (A2')).

	Similarly,
	\[
		\frac{\partial a_\mu^*}{\partial\psi_\mu}
		\;=\; -\frac{1}{\det J}\Bigl[ -\psi_\mu\,\mathcal M_{\sigma}\,\partial_{\psi_\mu}F_1 \;-\;\bigl(\kappa_\sigma-\varphi\,\mathcal S_{\sigma}\bigr)\,\partial_{\psi_\mu}F_2\Bigr].
	\]
	If, in addition, $\mathcal M_{\sigma}\le0$, then $-\psi_\mu\,\mathcal
		M_{\sigma}\ge0$, $\partial_{\psi_\mu}F_1\ge0$, $\kappa_\sigma-\varphi\,\mathcal
		S_{\sigma}>0$ by (A1), and $\partial_{\psi_\mu}F_2<0$, the bracket is positive;
	hence $\partial a_\mu^*/\partial\psi_\mu\ge0$ (strict under strict
	inequalities). This proves the claim.
\end{proof}
...
\subsection{Mean attention and price pivot}\label{app:mean_attention_pivot}

\begin{proposition}
	Let $\Delta V(y,B';a_\mu^{\mathrm{eff}})$ be continuous and preserve the
	baseline monotonicity in $(y,B')$ uniformly in $a_\mu^{\mathrm{eff}}$. Under
	the operator conditions in Appendix~\ref{app:operator_analysis}, the
	single-crossing pivot of prices in $B'$ at $B^*(y)$, its monotonicity in $y$,
	and the deleveraging/high-threshold comparative statics continue to hold with
	$\theta$ replaced by $\theta_{\mathrm{RI}}(y,B')$ and $\Delta V$ replaced by
	$\Delta V(y,B';a_\mu^{\mathrm{eff}})$.
\end{proposition}

\begin{proof}
	For each fixed $a_\mu^{\mathrm{eff}}$ and $\theta$, the operator
	$\mathcal T_\theta$ is positive and order preserving, and the resolvent is
	positive. The logistic default probability $\tilde P_\theta$ is single-crossing
	in $B'$ whenever the argument $-\Delta V/(\theta\eta)$ is. Since the mapping
	$\Delta V\mapsto L(-\Delta V/(\theta\eta))$ is strictly decreasing and smooth,
	single-crossing in $\Delta V$ transfers to $\tilde P_\theta$, hence to
	prices. The operator inheritance arguments used for the baseline (and for the
	state-dependent $\theta_{\mathrm{RI}}$) carry through, preserving the pivot and
	its monotone comparative statics in $y$. Monotone shifts in
	$\Delta V(\cdot; a_\mu^{\mathrm{eff}})$ do not overturn single crossing.
\end{proof}
