...
Let the dispersion weight entering the same pricing operator be an increasing
function of attention to the dispersion signal:
\begin{equation}
	\theta_{\mathrm{RI}}(y,B') \;=\; \min\Big\{\,1 + \varphi\, w\!\big(a_\sigma(y,B')\big),\; \bar\theta\,\Big\},
	\qquad \varphi>0,\; w\in C^1,\; w'(\cdot)>0.
...
\noindent\textit{Example (closed form).} For $w(a)=a$, the first-order condition
below yields $a_\sigma(y,B')=\tfrac{\varphi}{\kappa_\sigma}\,\mathcal S(y,B')$, so that
\begin{equation}
...
\begin{equation}
	\varphi\,w'\!\big(a_\sigma(y,B')\big)\, \mathcal S(y,B') \;=\; \kappa_\sigma\, a_\sigma(y,B').
	\label{eq:foc_sigma_micro}
\end{equation}
Under $w'(\cdot)>0$ and $\kappa_\sigma>0$, the solution $a_\sigma(y,B')$ is increasing
in $\mathcal S(y,B')$; therefore $\theta_{\mathrm{RI}}(y,B')$ is also increasing in
$\mathcal S(y,B')$ by~\eqref{eq:theta_RI_micro}.

\begin{lemma}[Existence/uniqueness and continuity of attention]
	\label{lem:attention_solution}
	Assume $w\in C^1$ with $w'(a)>0$ for all $a\ge0$ and $\Phi$ is strictly convex.
	For each fixed $\mathcal S\ge0$, the equation~\eqref{eq:foc_sigma_micro} admits a
	unique solution $a_\sigma=a_\sigma(\mathcal S)\in\mathbb R_+$, and the mapping
	$\mathcal S\mapsto a_\sigma(\mathcal S)$ is continuous and strictly increasing.
	If $\mathcal S(\cdot)$ is continuous on $\mathsf S$, then $a_\sigma(\cdot)$ and
	$\theta_{\mathrm{RI}}(\cdot)$ are continuous on $\mathsf S$.
\end{lemma}

\begin{proof}
	Define $H(a;\mathcal S)\coloneqq \kappa_\sigma a-\varphi w'(a)\mathcal S$. For fixed
	$\mathcal S\ge0$, $H(\cdot;\mathcal S)$ is strictly increasing and continuous with
	$\lim_{a\to\infty}H(a;\mathcal S)=+\infty$ and $H(0;\mathcal S)\le0$, hence a unique
	root exists by the intermediate value theorem. Monotone comparative statics in
	$\mathcal S$ follows from the implicit function theorem since $\partial H/\partial a
		=\kappa_\sigma-\varphi w''(a)\mathcal S>0$ under strict convexity of $\Phi$ and bounded
	$w''$. Continuity is standard.
\end{proof}

\begin{proposition}[Operator properties and inherited comparative statics]
	\label{prop:RI_operator_micro}
	Suppose $\Phi$ is strictly convex and $w$ in~\eqref{eq:theta_RI_micro} is
	increasing. If $\mathcal T_\theta$ is positive and order-preserving for each
...
\begin{proof}
	...
	$\mathcal T_{\theta_{\mathrm{RI}}(\cdot)}$ by monotonicity of $w$ and continuity of
	$a_\sigma(y,B')$ implied by~\eqref{eq:foc_sigma_micro}. Fixed-point arguments on
	compact grids then match Appendix~\ref{app:operator_analysis}.
\end{proof}
