...
I microfound PRO via rational inattention. Lenders optimally allocate attention
to dispersion, generating a state‑dependent tail weight
$\theta_{\mathrm{RI}}(y,B')$ that enters the \emph{same} pricing operator; all
baseline comparative statics (pivot, $B^*(y)$ increasing, deleveraging with
higher mean spreads) extend mechanically. Empirically, the framework matches
event‑style patterns such as Argentina’s inflation misreporting: sharper
pivots, higher default thresholds with deleveraging, and maturity/peer
decoupling concentrated in stress.

Roadmap. Section~\ref{sec:model} presents the environment and recursive
equilibrium; Section~\ref{sec:theory} develops core comparative statics;
...
Section~\ref{sec:micro} develops a microfoundation via rational inattention to
second moments; Section~\ref{sec:extensions} embeds policy and information
extensions (Ramsey planning, endogenous beliefs, optimal communication). The
appendix collects proofs and operator analysis.
...
formulation yields identical monotone comparative statics in precisions.)
...
(PRO)—with standard default environments through a single pricing operator. The
second‑moment distortion pivots price/spread schedules, so a rational sovereign
deleverages while defaulting at higher debt, raising average spreads and
generating a “stability illusion” (lower volatility with higher premia).

The rational‑inattention microfoundation makes PRO state‑dependent but remains
within the same operator: attention to dispersion yields
$\theta_{\mathrm{RI}}(y,B')$, and all baseline comparative statics carry over.
Policy and information extensions clarify limits and levers: Ramsey transfers
cannot undo pricing wedges; negativity‑biased learning sustains PRO; and
transparency lowers $\theta_{\mathrm{RI}}$ and raises welfare. The framework
offers compact, testable implications (pivot, threshold, decoupling) aligned
with episodes such as Argentina’s inflation misreporting, and provides a
...
This appendix collects auxiliary results, proofs, and computational details. Section~\ref{app:gumbel} details the Gumbel distribution and the logit identities used in the text. Section~\ref{app:proofs} provides proofs for the lemmas and propositions. Section~\ref{app:operator_analysis} states contraction, positivity/order, and differentiability results for the pricing operator (including Assumption RI.1 and a uniform convergence argument). Section~\ref{app:ri_general} develops the rational-inattention mapping with a general monotone $w(\cdot)$, establishing existence/uniqueness, continuity, and operator inheritance of the main comparative statics. Section~\ref{app:computations} documents the computational algorithm and the numerical stability techniques for the discrete-choice implementation.
