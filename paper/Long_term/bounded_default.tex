...
$s=(y,B')$ denote the relevant state. For operator properties used here and in
...
\]
In particular, at the margin $z=0$, the slope satisfies
$\E[L'(\varepsilon)] = \tfrac{1}{4}\big(1-\tfrac{\nu}{4}\big) + o(\nu)$,
which matches a logit with temperature $\tilde\theta$ via
$L'_\theta(0)=\tfrac{1}{4\theta}$. Identifying $\nu=\Var(\varepsilon)=\Var(\xi)/\eta^2$ for an implied uncertainty
on $\Delta V$, small posterior dispersion is locally equivalent to evaluating
the same operator at $\theta=\tilde\theta>1$.
\end{lemma}
...
two‑point illustration: if $\Delta V\in\{+m,-m\}$ with posterior weight
$\pi$ on $+m$, then the contribution to $\mathcal S$ is
